...
function updateIPReputation(ip: string, violation: string) {
  ...
  reputation.score = Math.max(0, reputation.score - (scoreDecrements[violation] || 5));

  // Block if score is too low
  if (reputation.score < 20) {
    reputation.blocked = true;
    reputation.blockedUntil = Date.now() + (30 * 60 * 1000); // Block for 30 minutes
  }
