...
// Helper function to get real IP address from proxy headers (prefers IPv4)
function getRealIP(req: Request): string {
  const allPossibleIPs: string[] = [];
  
  // Collect all possible IP addresses from different headers
  const doConnectingIP = req.headers['do-connecting-ip'] as string;
  if (doConnectingIP) allPossibleIPs.push(doConnectingIP);
  
  const cfConnectingIP = req.headers['cf-connecting-ip'] as string;
  if (cfConnectingIP) allPossibleIPs.push(cfConnectingIP);
...
export async function registerRoutes(app: Express): Promise<{ httpServer: Server; wss: WebSocketServer; startGames: () => void }> {
  ...
  app.post('/api/coin-flip/play', requireAuth, async (req, res) => {
    try {
      ...
      if (parseFloat(user.balance) < totalRequired) {
        return res.status(400).json({ 
          message: `Insufficient balance. You need $${totalRequired.toFixed(2)} (bet: $${betAmount.toFixed(2)} + ${feePercentage}% fee: $${feeAmount.toFixed(2)})` 
        });
      }

      // Store old balance for broadcasting
      const oldBalance = user.balance;
      
      // Deduct bet amount + fee upfront
      const balanceAfterBet = (parseFloat(user.balance) - betAmount - feeAmount).toFixed(8);
      await storage.updateUserBalance(userId, balanceAfterBet);
      ...
      if (won) {
        // Player wins - add full payout (2x bet amount)
        winAmount = betAmount * 2;
        finalBalance = (parseFloat(balanceAfterBet) + winAmount).toFixed(8);
      } else {
        // Player loses - balance already deducted, no change needed
        finalBalance = balanceAfterBet;
      }

      // Update balance once
      await storage.updateUserBalance(userId, finalBalance);
      ...
      broadcastBalanceUpdate(userId, oldBalance, finalBalance, won ? 'win' : 'loss');

      // Distribute commissions on the bet amount
      await distributeCommissions(userId, betAmount);

      // Save coin flip game to database
  ...
  async function updateGoldenLivePlayers() {
    try {
      ...
      if (currentStats) {
        ...
        console.log(`📊 [Golden Live] Total players updated: ${currentStats.totalPlayers} → ${newTotal} (+${increment})`);
      }
    } catch (error) {
