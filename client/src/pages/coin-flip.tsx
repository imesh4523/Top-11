  ...
  const flipCoinMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      flipTimeoutRef.current = setTimeout(() => {
        ...
        // Refresh game history from database
          ...
          <CardContent className="p-4">
            ...
            <div className="mt-4 p-3 bg-gradient-to-r from-green-500/20 to-emerald-500/20 rounded-lg border border-green-500/30">
              <div className="flex items-center justify-between">
                <div className="text-sm text-white/80">Potential Win:</div>
                <div className="flex items-center gap-2">
                  <TrendingUp className="w-4 h-4 text-green-400" />
                  <div className="text-lg font-bold text-green-400">
                    {formatGoldCoins(betAmount * 2)}
                  </div>
                </div>
              </div>
            </div>
                    ...
                    >
                      ...
                      <div className="text-right">
                        <div className={`font-bold ${game.won ? 'text-green-400' : 'text-red-400'}`}>
                          {game.won ? '+' : '-'}{formatGoldCoins(game.won && game.winAmount ? usdToGoldCoins(parseFloat(game.winAmount)) : usdToGoldCoins(parseFloat(game.betAmount)))}
                        </div>
                        <div className="text-xs text-white/60">
